...
import jobQueries from '../pages/api/jobs';

describe('/api/jobs', () => {
  it('should retrieve all jobs', async () => {
    const { req: request, res: response } = createMocks();
    const jobs = await jobQueries(request, response);
  ...
  it('should sort job items by sort key and direction', async () => {
    const { req: request, res: response } = createMocks({ url: '/api/jobs?keyword=miller&sortType=experience&sortDirection=asc' });
    const jobs = await jobQueries(request, response);
