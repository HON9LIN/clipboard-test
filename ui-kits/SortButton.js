...
const SortButton = ({ label, className = '', state, setState, sortType, activeKey }) => {

  const cycleState = () => {
    if (activeKey !== sortType) {
      setState('asc');
      return;
    }
  ...
  return (<button className={`${className} focus:outline-none relative`} onClick={() => cycleState()}>
    ...
    <span>{ activeKey === sortType && state === 'asc' && <ArrowDown/> }
      { activeKey === sortType && state === 'desc' && <ArrowUp/> }</span>
    </div>
  </button>);
}
